...
def LinearProcessingElement():
    return verilog.from_verilog.read_verilog_module(
        os.path.join(TOP_DIR,'rtl/LinearProcessingElementWrapper.v')
    )['LinearProcessingElementWrapper'] 

def tb_LinearProcessingElement(I=1,J=1,i=0,j=0):
    module = Module(f'tb_LinearProcessingElement_{I}_{J}_{i}_{j}')
    ...
    vcd_name = os.path.join('..','vcd',f'tb_LinearProcessingElement_{I}_{J}_{i}_{j}.vcd')
...
def tb_LinearProcessingElement_v2(I=1,J=1,i=0,j=0):
    module = Module(f'tb_LinearProcessingElement_v2_{I}_{J}_{i}_{j}')
...
def main():
    ...
    for I,J,i,j in (
        ...
        test = tb_LinearProcessingElement(I=I,J=J,i=i,j=j)
        fname_base=f'tb_LinearProcessingElement_{I}_{J}_{i}_{j}'
        ...
        # test = tb_LinearProcessingElement_v2(I=I,J=J,i=i,j=j)
        fname_base=f'tb_LinearProcessingElement_v2_{I}_{J}_{i}_{j}'
        # fname = os.path.join(TOP_DIR,f'tb/{fname_base}.v')
        # verilog_test = test.to_verilog(fname)
        # stripModule(fname, 'LinearProcessingElement')
        # addTimeScale(fname)
