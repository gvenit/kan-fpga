...
def Sech2Lutram() -> Module:
    return verilog.from_verilog.read_verilog_module(
        os.path.join(TOP_DIR,'rtl/wrapper/AxisRomWrapper.v')
    )['AxisRomWrapper']
...
def tb_Sech2Lutram():
    ...
    params['ADDR_WIDTH'].value = DATA_WIDTH
    ...
    per_channel.Always(Posedge(clk))(
        If(reset_done)(
            ...
            If(Ors(
                ...
                m_axis_tready[CHANNELS-1-chn](0)
            ),
            If(Not(m_axis_tready[chn]))(
                m_axis_tready[chn](1)
            ),
            ...
            If(Ands(s_axis_tlast[chn],s_axis_tready[chn],s_axis_tvalid[chn]))(
                s_axis_tlast[chn](0)
            ),
            If(Ands(s_axis_tready[chn],s_axis_tvalid[chn]))(
                data_reg(data_next)
            ),
        ).Else(
            data_reg(chn)
