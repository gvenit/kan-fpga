...
def SubMultAbs() -> Module:
    return verilog.from_verilog.read_verilog_module(
        os.path.join(TOP_DIR,'rtl/SubMultAbs.v')
    )['SubMultAbs']

def tb_SubMultAbs():
    module = Module('tb_SubMultAbs')

    subMultAbs = SubMultAbs()
    ...
    uut = module.Instance(
        ...
        'uut',
        params,
        # module.copy_sim_ports(SubMultAbs)
        ports
    )
    # print(ports)
    clk = ports['clk']
    ...
    vcd_name = os.path.join('..','vcd','tb_SubMultAbs.vcd')
...
def main():
    ...
    SubMultAbs().to_verilog(os.path.join(TOP_DIR,'rtl/tmp.v'))

    test = tb_SubMultAbs()
    fname = os.path.join(TOP_DIR,'tb/tb_SubMultAbs.v')
    verilog_test = test.to_verilog(fname)
    
    stripModule(fname, 'tb_SubMultAbs')
    addTimeScale(fname)
    ...
    #     ],
    #     outputfile=os.path.join(TOP_DIR,'out/tb_SubMultAbs.out'),
    # )
    # print(rslt)
