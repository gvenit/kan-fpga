...
int main(void)
{
    ...
    for (uint8_t layer = 0; layer < hKan.layers_num; layer++)
    {
        ...
        if ((kan_state != KAN_STATE_ITRL_ERR) && (kan_state != KAN_STATE_OPER_ERR)) // !!!!!!!!!! for debug pusposes it is &&, it should be ||
        {
        ...
        kan_mem_reg_write(KAN_REG_BASE_ADDRESS, KAN_REG_OFST_INTR_1B, KAN_REG_MASK_INTR_SFT_1B, BYTE);
