...
kan_status_t kan_dma_attach_irq(kan_dma_handler_t *dma_handler, kan_intr_handler_t *intr_handler)
{
    ...
    status = kan_intr_attach(intr_handler, TX_INTR_ID, TX_INTR_PRIORITY, TX_INTR_TRIGGER_TYPE, (kan_intr_callback_t)callback_dma_tx, (void *)dma_handler);
    if (status != STATUS_OK)
        return STATUS_INTR_ATTACH_FAILURE;

    status = kan_intr_attach(intr_handler, RX_INTR_ID, RX_INTR_PRIORITY, RX_INTR_TRIGGER_TYPE, (kan_intr_callback_t)callback_dma_rx, (void *)dma_handler);
    if (status != STATUS_OK)
        return STATUS_INTR_ATTACH_FAILURE;
